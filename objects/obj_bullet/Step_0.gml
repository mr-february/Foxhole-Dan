...
// Sub-step the movement so fast bullets (e.g. the sniper's speed-28 rounds)
// can't tunnel straight through a thin enemy hitbox by jumping past it in
// one frame — collision is checked after every few pixels of travel instead
// of only once at the final position.
var _steps     = max(1, ceil(speed / 6));
var _step_dist = speed / _steps;

for (var _i = 0; _i < _steps; _i++) {
    x += lengthdir_x(_step_dist, direction);
    y += lengthdir_y(_step_dist, direction);
    ...
    if (hit != noone && hit.object_index != obj_boss) {
        ...
        if (_hif == 0) {
            // Headshot zone — top ~27% of the enemy's actual current bounding box,
            // so this scales correctly across every enemy shape/size, not just the
            // 18x30 soldier mask. Always an instant kill.
            var _headband = hit.bbox_top + (hit.bbox_bottom - hit.bbox_top) * 0.27;
            var _headshot = (y <= _headband);
            ...
            if (_headshot) {
                ...
                var hdmg = instance_create_layer(hit.x, hit.y - 30, "Instances", obj_damage_number);
                hdmg.label = "HEADSHOT";
                scr_award_kill(hit, _sv);
                scr_spawn_headshot_gore(hit.x, hit.y - 26, _fc);
                global.shake_mag = max(global.shake_mag, 12.0);
                ...
                if (hit.hp <= 0) {
                    var _sv2 = variable_instance_exists(hit, "score_value") ? hit.score_value : 100;
                    var _fc2 = variable_instance_exists(hit, "facing") ? hit.facing : 1;
                    scr_award_kill(hit, _sv2);
                    scr_spawn_gore(hit.x, hit.y, _fc2);
                    instance_destroy(hit);
                } else {
