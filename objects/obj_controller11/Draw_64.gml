...
// === CUTSCENE — hand off (defensive; no cutscene spawns in this room today) ===
...
if (global.game_state == 1) {
    ...
    draw_text_transformed(gw/2, gh/2 - 60, "LEVEL " + string(level_no) + " COMPLETE", 2.5, 2.5, 0);
    draw_set_color(c_white);
    draw_text_transformed(gw/2, gh/2, "The Handler is down.", 1.2, 1.2, 0);
    draw_text_transformed(gw/2, gh/2 + 40, "MK-ECHO knows you're inside.", 1.0, 1.0, 0);
    ...
    draw_text_transformed(gw/2, gh/2 + 100, "Press R to replay", 0.85, 0.85, 0);
...
if (global.game_state == 2) {
    ...
    draw_text_transformed(gw/2, gh/2 + 20, "The facility keeps its secrets.", 1.1, 1.1, 0);
...
// --- BOSS HEALTH BAR (top center) — THE HANDLER ---
var boss = instance_find(obj_boss_handler, 0);
if (boss != noone) {
    ...
    draw_set_halign(fa_center);
    draw_set_color(c_white);
    var boss_label = (boss.phase == 2) ? "THE HANDLER  [ENRAGED]" : "THE HANDLER";
...
if (gamepad_is_connected(0)) {
    ...
    draw_text(16, leg_y,      "WASD/Arrows Move  |  Space Jump  |  J/LMB Shoot  |  Move keys Aim  |  K Grenade");
}
draw_set_alpha(1);
...
// Kill flash — brief white punch on every kill
