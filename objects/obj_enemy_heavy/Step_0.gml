...
if (instance_exists(obj_dan)) {
    ...
    if (dist < aggro_range) {
        // Advance menacingly — slow, relentless, holds at ledge edges
        ...
        // Slow heavy cannon shot
        if (shoot_timer > 0) shoot_timer--;
        if (shoot_timer <= 0 && dist < shoot_range) {
            var b = instance_create_layer(x, y - 22, "Instances", obj_enemy_bullet);
            b.direction   = point_direction(x, y - 22, p.x, p.y - 10);
            b.speed       = 4.5;
